...
from app.models_pydantic import SportOut, SportCreate, SportUpdate
...
@router.post("", response_model=SportOut, status_code=status.HTTP_201_CREATED, summary="ADD new sport activity (Admin CRUD)")
def create_sport(sport: SportCreate):
    """Adds a new sport activity document to Google Firestore."""
    return crud.create_sport(sport_data=sport)

@router.put("/{sport_id}", response_model=SportOut, summary="EDIT / UPDATE existing sport activity (Admin CRUD)")
def update_sport(sport_id: str, sport: SportUpdate):
    """Updates/edits an existing sport activity document in Google Firestore."""
    return crud.update_sport(sport_id=sport_id, sport_data=sport)

@router.delete("/{sport_id}", status_code=status.HTTP_200_OK, summary="DELETE / Deactivate sport activity (Admin CRUD)")
def delete_sport(sport_id: str):
    """Deletes / deactivates a sport activity document from Google Firestore."""
